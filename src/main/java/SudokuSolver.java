...
public class SudokuSolver { // T = O(N^3) | Class that contains methods to solve the given sudoku grid
    ...
    SudokuSolver(char[][] grid) { // of course we'll need the grid that has to be solved as an argument
        if (solve(grid, 0, 0)) Main.printGrid(this.grid); // if solve process is true, than we have to print the fully-solved grid
    }
    ...
    private boolean solve(char[][] grid, int row, int col) { // T = O(N^3) | recursive function that solves the grid according to given row and column value as arguments
        ...
        if (grid[row][col] != DEFAULT) return solve(grid, row, col + 1); // and of course if the cell in present is not blank, we have to exceed to the next
        ...
        for (int index = 0; index < 9; index++) { // T = O(N^3) |  we'll use brute force approach means we'll pick every single one value and try it for the cell in present
            ...
            if (isAppropriate(grid, row, col, value)) { // T = O(N^2) | we're controlling is this value appropriate for this cell or not
                ...
                if (solve(grid, row, col + 1)) return true; // and we'll continuing to the next value till the grid is fully solved
    ...
    private boolean isAppropriate(char[][] grid, int row, int col, char value) { //
        ...
        for (int r = 0; r < 3; r++) { // T = O(N^2)
            for (int c = 0; c < 3; c++) {
                if (value == grid[gridRow + r][gridColumn + c]) return false; // and we're checking the grids if there is a match of given value
            }
        }
